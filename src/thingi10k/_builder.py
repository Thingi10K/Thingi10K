...
class Thingi10KBuilder(datasets.GeneratorBasedBuilder):
    ...
    def _split_generators(self, dl_manager):
        ...
        metadata_url = f"{repo_url}/metadata"
        ...
        tags_csv = dl_manager.download(f"{metadata_url}/tag_data.csv")
        assert pathlib.Path(tags_csv).exists()
        ...
        tag_data = pl.read_csv(
            tags_csv, schema_overrides=tag_schema, ignore_errors=True
        )
        ...
        if self.config.name == "npz":
            ...
            downloaded_files = [
                extraction_dir / "npz" / f"{file_id}.npz" for file_id in file_ids if file_id not in _corrupt_file_ids
            ]
            ...
            downloaded_files = [
                ...
                if row[0] not in _corrupt_file_ids
            ]
        else:
    ...
    def _generate_examples(
        self, downloaded_files, geometry_data, contextual_data, summary_data, tag_data
    ):
        """
        ...
        df = geometry_data.join(
            thing_file_ids, left_on="file_id", right_on="ID", how="left"
        )
        df = df.join(contextual_data, on="Thing ID", how="left")

        tag_data = tag_data.group_by("Thing ID").agg(pl.col("Tag").alias("Tags"))
        df = df.join(tag_data, on="Thing ID", how="left")
        ...
        df = df.with_columns(
            [
                ...
                df["Tags"].fill_null(pl.lit([])).alias("Tags"),
            ]
        )
